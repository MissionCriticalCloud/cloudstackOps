...
# Class with Cloudstack storage and Hypervisor storage methods
...
class StorageHelper():
    ...
    def _remote_cmd(self, hostname, cmd):
        ...
        try:
            ...
            if result:
                if self.debug > 0:            
                    print "[DEBUG]: CMD SUCCES:", result.succeeded, "CMF FAILED:", result.failed, "CMD RETCODE:", result.return_code, "CMD ERR:", result.stderr
    ...
    def get_mountpoint(self, hostname, device_path):
        ...
        if len(mount_list) > 0:
            for mount_point, mount_device in mount_list.iteritems():
                             
                if mount_device.endswith('/'):
                    # strip the slash
    ...
    def list_files(self, hostname, path):
        ...
        if path is not '':
            ...
            if returncode == 0:
                ...
                print "[ERROR]: Failed to retrieve list of file due to: ", errmsg
